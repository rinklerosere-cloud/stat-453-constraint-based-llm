"""
LoRA fine-tuning script for RECAST constraint-following experiments.

Usage:
    python lora_base.py [options]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="LoRA fine-tuning on RECAST constraint-following data with K-Fold CV."
    )
    p.add_argument("--model_name",   default=DEFAULT_MODEL)
    p.add_argument("--dataset",      default=DEFAULT_DATASET)
    p.add_argument("--output_dir",   default="./output/lora_finetuned")
    ...
    p.add_argument("--target_modules", nargs="+", default=["q_proj", "v_proj"])
    return p.parse_args()
...
def main() -> None:
    ...
    for fold_num, (train_idx, val_idx) in enumerate(kf.split(records), start=1):
        ...
        training_args = TrainingArguments(
            ...
            label_names                 = ["labels"],
            run_name                    = f"lora_ft_{safe_model_name}_fold{fold_num}",
        )
